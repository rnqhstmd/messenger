...
public class MessageController {
    ...
    // 보낸 메세지 목록 조회
    @GetMapping("/sent")
    public ResponseEntity<ResponseDto<MessagePreviewListResponseData>> getSentMessages(@OrganizationMember User user) {
        MessagePreviewListResponseData sentMessages = messageService.getSentMessages(user);
        return new ResponseEntity<>(ResponseDto.success(HttpStatus.OK, "보낸 메세지 조회 완료", sentMessages), HttpStatus.OK);
    }

    // 받은 메세지 목록 조회
    @GetMapping("/received")
    public ResponseEntity<ResponseDto<MessagePreviewListResponseData>> getReceivedMessages(@OrganizationMember User user) {
        MessagePreviewListResponseData receivedMessages = messageService.getReceivedMessages(user);
        return new ResponseEntity<>(ResponseDto.success(HttpStatus.OK, "받은 메세지 조회 완료", receivedMessages), HttpStatus.OK);
    }

    // 보낸 메세지 읽기
    @GetMapping("/sent/{id}")
    public ResponseEntity<ResponseDto<MessageResponseData>> readReceivedMessage(@OrganizationMember User user,
                                                                                @PathVariable("id") UUID messageId) {
        MessageResponseData messageResponseData = messageService.readSentMessage(user, messageId);
        return new ResponseEntity<>(ResponseDto.success(HttpStatus.OK, "메세지 읽기 완료", messageResponseData), HttpStatus.OK);
    }

    // 받은 메세지 읽기
    @GetMapping("/received/{id}")
    public ResponseEntity<ResponseDto<MessageResponseData>> readSentMessage(@OrganizationMember User user,
                                                                            @PathVariable("id") UUID messageId) {
        MessageResponseData messageResponseData = messageService.readReceivedMessage(user, messageId);
        return new ResponseEntity<>(ResponseDto.success(HttpStatus.OK, "메세지 읽기 완료", messageResponseData), HttpStatus.OK);
    }

    // 메세지 답장
    @PostMapping("received/{id}/reply")
